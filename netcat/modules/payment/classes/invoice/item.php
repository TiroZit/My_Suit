...
class nc_payment_invoice_item extends nc_record {
    ...
    // значения как для реквизита «признак предмета расчёта» ФФД (тег 1212)
    const TYPE_COMMODITY = 1;
    const TYPE_EXCISED_COMMODITY = 2;
    const TYPE_JOB = 3;
    const TYPE_SERVICE = 4;
    const TYPE_INTELLECTUAL_ACTIVITY = 9;
    const TYPE_PAYMENT = 10;
    const TYPE_COMPOSITE = 12;
    const TYPE_OTHER = 13;
    ...
    static protected $allowed_vat_rates = array(null, 0, 10, 20);
    /** @var int ставка НДС, когда передано недопустимое значение */
    static protected $default_vat_rate = 20;
    ...
    public function __construct($values = null) {
        ...
        if (date('Y') < 2019) {
            self::$default_vat_rate = 18;
            self::$allowed_vat_rates = array(null, 0, 10, 18);
        }
    }
