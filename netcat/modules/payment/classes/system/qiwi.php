...
HERE;
    ...
    public function get_request_form(nc_payment_invoice $invoice, $show = true, $open_in_new_window = true) {
        ...
        if ($AUTH_USER_ID && $invoice->get('customer_id') == $AUTH_USER_ID) {
            $invoice_phone = htmlspecialchars(nc_normalize_phone_number($invoice->get('customer_phone')), ENT_QUOTES);
        }

        $target = ($open_in_new_window ? " target='_blank'" : '');

        $result = "<form action='{$this->get_request_script_path()}' method='post' $target id='nc_payment_form_qiwi'>";
        ...
        $result .= "<input id='nc_payment_form_qiwi_phone' type='text' name='param_PhoneNumber' placeholder='+7__________' value='$invoice_phone' autofocus>";
