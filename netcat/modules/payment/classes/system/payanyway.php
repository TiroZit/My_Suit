...
class nc_payment_system_payanyway extends nc_payment_system {
    ...
    static protected $payanyway_vat_enum = array(
        ...
        10 => 1103, // НДС 10%
        18 => 1102, // НДС 18%
        20 => 1102, // НДС 20% (то же значение, что для 18%)
    );
    static protected $payanyway_default_vat = 1102;

    static protected $payanyway_payment_object_enum = array(
    ...
    public function process_receipt(nc_payment_receipt $receipt) {
        ...
        foreach ($items as $item) {
            $inventory_positions[] = array(
                ...
                'pm' => 'full_prepayment', // предоплата 100%
                'po' => nc_array_value(self::$payanyway_payment_object_enum, $item->get('type'), 'commodity'),
            );
        }
