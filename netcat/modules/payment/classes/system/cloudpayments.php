...
class nc_payment_system_cloudpayments extends nc_payment_system {
    ...
    public function __construct() {
        parent::__construct();
        if (date('Y') < 2019) {
            self::$cloudpayments_default_vat = '18';
            self::$vat_map = array('' => '', 0 => '0', 10 => '10', 18 => '18');
        }
    }
    ...
    private function get_receipt(nc_payment_invoice $invoice) {
        ...
        foreach ($items as $item) {
            $receipt_data['Items'][] = array(
                ...
                'object'   => $item->get('type'), // значения для тега 1212 совпадают с nc_payment_invoice_item::TYPE_*
            );
        }
    ...
    protected function on_response(nc_payment_invoice $invoice = null) {
        ...
        switch ($action) {
            ...
            case 'fail':
                ...
                break;
            case 'refund':
                return; // ничего не делаем в случае с refund
        }
