...
class nc_payment_system_paymaster extends nc_payment_system {
    ...
    static protected $paymaster_default_vat = 'vat20';
    ...
    public function __construct() {
        parent::__construct();
        if (date('Y') < 2019) {
            self::$paymaster_default_vat = '18';
            self::$paymaster_vat_enum = array('' => 'no_vat', 0 => 'vat0', 10 => 'vat10', 18 => 'vat18');
        }
    }
        ...
        if ($this->get_setting('SEND_RECEIPTS')) {
            ...
            if ($sell_receipt) {
                foreach ($sell_receipt->get_items() as $item) {
                    ...
                    $inputs["LMI_SHOPPINGCART.ITEM[$i].SUBJECT"] = $item->get('type'); // совпадает со значениями nc_payment_invoice_item::TYPE_*
                    $i++;
                }
