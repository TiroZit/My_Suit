...
abstract class nc_payment_system {
     ...
     * @return string|null
     */
    public function get_response_value() {
        $args = func_get_args();
        $value = $this->callback_response;
        foreach ($args as $key) {
            if (isset($value[$key])) {
                $value = $value[$key];
            } else {
    ...
    /**
     * Удаляет невалидные UTF8-последовательности из элементов массива
     * @param array $original_array
     * @return array
     */
    static private function clean_utf8(array $original_array) {
        // Если в ответе содержится некорректные с точки зрения UTF8 последовательности
        // (например, когда PayPal не настроен на его стороне), json_encode возвратит false.
        // Уберём не-UTF8-последовательности — лучше записать хоть что-то, чем ничего.
        $clean_array = array();
        foreach ($original_array as $key => $value) {
            $clean_array[$key] = is_array($value) ? self::clean_utf8($value) : mb_convert_encoding($value, "UTF-8", "UTF-8");
        }
        return $clean_array;
    }
    ...
    final public function process_callback_response(array $response = array()) {
        ...
        if ($invoice instanceof nc_payment_invoice) {
            $response_to_log = self::clean_utf8($this->get_response() ?: array());
    ...
    protected function on_payment_success(nc_payment_invoice $invoice = null) {
        if ($invoice) {
            if ($invoice->get('status') == nc_payment_invoice::STATUS_SUCCESS) {
                return; // ничего не делать при повторном вызове on_payment_success()
            }
