...
class nc_payment_register_provider_kkmserver extends nc_payment_register_provider {
    ...
    static protected $kkm_default_vat = 20;
    ...
    public function __construct($site_id = null, $provider_id = null) {
        ...
        if (date('Y') < 2019) {
            self::$kkm_default_vat = 18;
            self::$kkm_vat_enum = array('' => -1, 0 => 0, 10 => 10, 18 => 18);
        }
    }
    ...
    public function process_receipt(nc_payment_receipt $receipt) {
        ...
        $data = array(
            ...
            'SenderEmail' => $this->get_setting('PaymentRegisterEmail'),
            // Строки чека
            'CheckStrings' => array(
                array(
                    'PrintText' => array(
            ...
            'ElectronicPayment' => $totals, // Сумма электронной оплаты
        );
        ...
        foreach ($items as $item) {
            $goods_data[] = array(
                'Register' => array(
                    ...
                    'Department' => 0,
                    // НДС в процентах или ТЕГ НДС: 0 (НДС 0%), 10 (НДС 10%), 20 (НДС 20%), -1 (НДС не облагается), 118 (НДС 18/118), 110 (НДС 10/110):
                    'Tax' => nc_array_value(self::$kkm_vat_enum, $item->get('vat_rate'), self::$kkm_default_vat),
                    'SignMethodCalculation' => 1, // предоплата 100%
                    'SignCalculationObject' => $item->get('type'), // в константах и в KKM Server используются одинаковые значения (ФНС)
                ),
            );
        }
