...
class nc_payment_register_provider_cloudkassir extends nc_payment_register_provider {
    ...
    public function __construct($site_id = null, $provider_id = null) {
        ...
        if (date('Y') < 2019) {
            self::$cloudkassir_default_vat = '18';
            self::$cloudkassir_vat_enum = array('' => '', 0 => '0', 10 => '10', 18 => '18');
        }
    }
    ...
    public function process_receipt(nc_payment_receipt $receipt) {
        ...
        foreach ($items as $item) {
            $data['CustomerReceipt']['Items'][] = array(
                ...
                'object'   => $item->get('type'), // в константах и в CloudKassir используются одинаковые значения (ФНС)
            );
        }
