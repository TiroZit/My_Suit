...
class nc_payment_invoice extends nc_record {
    ...
    public function save() {
        parent::save();
        $original_amount = $this->get('amount');
        $this->save_items();
        // общая цена могла измениться, если нулевая цена была заменена на 0.01; надо пересохранить стоимость в заказе
        if ($original_amount != $this->get('amount')) {
            parent::save();
        }
        return $this;
    }
    ...
    protected function get_dummy_invoice_item() {
        return new nc_payment_invoice_item(array(
            ...
            'type' => nc_payment_invoice_item::TYPE_COMMODITY,
    ...
    protected function get_items_from_netshop_order() {
        ...
        $items = new nc_payment_invoice_item_collection();
        ...
        foreach ($order_items as $order_item) {
            ...
            $items->add(new nc_payment_invoice_item(array(
                'invoice_id' => $invoice_id,
                'operation' => nc_payment::OPERATION_SELL,
                'name' => $order_item['FullName'] ?: "[$order_item[_ItemKey]]",
                'item_price' => max(0.01, $order_item['ItemPrice']), // цена может быть 0 в заказе (скидка 100%), но должна быть ненулевой в чеках
                'qty' => $order_item['Qty'],
                'vat_rate' => $vat_rate,
        ...
        if ($order['DeliveryPriceWithDiscount'] > 0) {
            $items->add(new nc_payment_invoice_item(array(
                'invoice_id' => $invoice_id,
                'operation' => nc_payment::OPERATION_SELL,
                'type' => nc_payment_invoice_item::TYPE_SERVICE,
