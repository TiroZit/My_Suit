...
class nc_payment {
    ...
    // признаки способов расчёта (в скобках — «значения реквизита» для тега ФФД 1214):
    /** полная предварительная оплата до момента передачи предмета расчёта — «предоплата 100%» (1) */
    const PAYMENT_METHOD_FULL_PREPAYMENT = 1;
    /** частичная предварительная оплата до момента передачи предмета расчёта — «предоплата» (2) */
    const PAYMENT_METHOD_PARTIAL_PREPAYMENT = 2;
    /** аванс (предмет расчёта на момент оплаты точно не известен) — (3) */
    const PAYMENT_METHOD_ADVANCE = 3;
    /** полная оплата, в том числе с учётом аванса (предварительной оплаты) в момент передачи предмета расчёта — «полный расчёт» (4) */
    const PAYMENT_METHOD_FULL_PAYMENT = 4;
    /** частичная оплата предмета расчёта в момент его передачи с последующей оплатой в кредит — «частичный расчёт и кредит» (5) */
    const PAYMENT_METHOD_PARTIAL_PAYMENT = 5;
    /** передача предмета расчёта без его оплаты в момент его передачи с последующей оплатой в кредит — «передача в кредит» (6) */
    const PAYMENT_METHOD_CREDIT = 6;
    /** оплата предмета расчёта после его передачи с оплатой в кредит — «оплата кредита» (7) */
    const PAYMENT_METHOD_CREDIT_PAYMENT = 7;
