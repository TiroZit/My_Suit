...
function banner_NavBar() {
    ...
    if ($phase) {
        ...
        echo "<tr><td><a href='?export_excel=1&".http_build_query($_GET, null, '&amp;')."'>".NETCAT_MODULE_BANNER_EXCEL_EXPORT."</a></td><td><font size=-2>";

        $link_pref = "?phase=".(!$phase ? 1 : $phase)."&date_end_d=".date("d")."&date_end_m=".date("m")."&date_end_y=".date("Y").($full ? "&full=" . (int)$full : "");
        echo "<a href=".$link_pref."&date_start_d=".$today_d."&date_start_m=".$today_m."&date_start_y=".$today_y.">".NETCAT_MODULE_BANNER_TODAY."</a>";
        echo " | <a href=?phase=".(!$phase ? 1 : $phase)."&date_end_d=".$yesterday_d."&date_end_m=".$yesterday_m."&date_end_y=".$yesterday_y."&date_start_d=".$yesterday_d."&date_start_m=".$yesterday_m."&date_start_y=".$yesterday_y.($full ? "&full=" . (int)$full : "").">".NETCAT_MODULE_BANNER_YESTERDAY."</a>";
    ...
    echo "<input type=hidden name=full value='" . ($full ? (int)$full : '') . "'>";
