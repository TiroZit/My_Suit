...
function banner_rotate($zone, $number=1, $rand=false) {
    ...
    for ($nmb = 1; $nmb <= $number; $nmb++) {
        ...
        $click_url = nc_get_scheme() . '://' . $HTTP_HOST . nc_module_path('banner') . "?b={$banner_id}&amp;c={$campaign_id}&amp;z={$zone}&amp;r={$url_for_stats}&amp;rnd={$random_value}&amp;click=1";
        $click_target = $banner_new_window ? "target='_blank' " : "";

        switch ($banner_type) {
            case 1:
                $code = "<a " . $click_target . " href='" . nc_module_path('banner') . "?b={$banner_id}&amp;c={$campaign_id}&amp;z={$zone}&amp;r={$url_for_stats}&amp;rnd={$random_value}&amp;click=1'>";
        ...
        $code .= "<img src='" . nc_module_path('banner') . "index.php?rnd={$random_value}&amp;c={$campaign_id}&amp;z={$zone}&amp;b={$banner_id}&amp;r={$url_for_stats}{$adm_mode}' width='1' height='1' alt='' />";
        $final_code .=$code;
    }
