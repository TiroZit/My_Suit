...
class nc_netshop {
    ...
    const NETSHOP_ORDER_COMPONENT_KEYWORD = 'netcat_module_netshop_order';
    const NETSHOP_ORDER_TEMPLATE_KEYWORD = 'single_page_checkout';
    const NETSHOP_ORDER_LIST_TEMPLATE_KEYWORD = 'list';
    const NETSHOP_ORDER_SUCCESS_COMPONENT_KEYWORD = 'netcat_module_netshop_checkout_completed';
    const NETSHOP_ORDER_SUCCESS_COMPONENT_TEMPLATE_KEYWORD = 'user_hash_payment';
    const NETSHOP_COMPARE_COMPONENT_KEYWORD = 'netcat_module_netshop_compare';
    const NETSHOP_FAVORITES_COMPONENT_KEYWORD = 'netcat_module_netshop_favorite';
    ...
    public function check_new_order(nc_netshop_order $order) {
        ...
        if (!$order->get_items()->count() && !$this->cart->get_items()->count()) {
    ...
    /**
     * Создаёт разделы для интернет-магазина (корзина, заказ, сравнение товаров, избранные товары)
     */
    public function create_netshop_subdivisions() {
        $site_id = $this->catalogue_id;

        $nc_core = nc_core::get_object();
        $order_component_id = $this->get_special_component_id(self::NETSHOP_ORDER_COMPONENT_KEYWORD);
        $order_template_id = $nc_core->component->get_component_template_by_keyword($order_component_id, self::NETSHOP_ORDER_TEMPLATE_KEYWORD, 'Class_ID');
        $cart_subdivision_id = $nc_core->catalogue->get_by_id($site_id, 'Netshop_Cart_Sub_ID');

        // раздела "Корзина" еще нет
        if (!$cart_subdivision_id) {
            if ($order_component_id) {
                // раздел с корзиной
                $cart_subdivision_id = $nc_core->subdivision->create(array(
                    'Catalogue_ID' => $site_id,
                    'Subdivision_Name' => NETCAT_MODULE_NETSHOP_CART_SUBDIVISION_NAME,
                    'EnglishName' => 'cart',
                    'Checked' => 0,
                ));

                $nc_core->sub_class->create($order_component_id, array(
                    'Subdivision_ID' => $cart_subdivision_id,
                    'Sub_Class_Name' => NETCAT_MODULE_NETSHOP_CART_SUBDIVISION_NAME,
                    'EnglishName' => 'cart',
                    'Class_Template_ID' => $order_template_id,
                ));

                $nc_core->set_settings('OrderComponentID', $order_component_id, 'netshop', $site_id);
            } else {
                $cart_subdivision_id = 0;
            }
        }

        $success_subdivision_id = $nc_core->catalogue->get_by_id($site_id, 'Netshop_Order_Success_Sub_ID');
        // раздела "Заказ оформлен" еще нет
        if (!$success_subdivision_id) {
            // раздел "Заказ оформлен"
            $order_success_component_id = $this->get_special_component_id(self::NETSHOP_ORDER_SUCCESS_COMPONENT_KEYWORD);
            $order_success_component_template_id = $nc_core->component->get_component_template_by_keyword($order_success_component_id, self::NETSHOP_ORDER_SUCCESS_COMPONENT_TEMPLATE_KEYWORD, 'Class_ID');
            if ($order_success_component_id) {
                $success_subdivision_id = $nc_core->subdivision->create(array(
                    'Catalogue_ID' => $site_id,
                    'Parent_Sub_ID' => $cart_subdivision_id,
                    'Subdivision_Name' => NETCAT_MODULE_NETSHOP_ORDER_SUCCESS_SUBDIVISION_NAME,
                    'EnglishName' => 'finish',
                    'Checked' => 0,
                ));

                $nc_core->sub_class->create($order_success_component_id, array(
                    'Subdivision_ID' => $success_subdivision_id,
                    'Sub_Class_Name' => NETCAT_MODULE_NETSHOP_ORDER_SUCCESS_SUBDIVISION_NAME,
                    'Class_Template_ID' => $order_success_component_template_id,
                    'EnglishName' => 'finish',
                ));
            } else {
                $success_subdivision_id = 0;
            }
        }

        $compare_subdivision_id = $nc_core->catalogue->get_by_id($site_id, 'Netshop_Compare_Sub_ID');
        // раздела "Сравнение товаров" еще нет
        if (!$compare_subdivision_id) {
            // раздел "Сравнение товаров"
            $compare_component_id = $this->get_special_component_id(self::NETSHOP_COMPARE_COMPONENT_KEYWORD);
            if ($compare_component_id) {
                $compare_subdivision_id = $nc_core->subdivision->create(array(
                    'Catalogue_ID' => $site_id,
                    'Subdivision_Name' => NETCAT_MODULE_NETSHOP_COMPARE_SUBDIVISION_NAME,
                    'EnglishName' => 'compare',
                    'Checked' => 0,
                ));

                $nc_core->sub_class->create($compare_component_id, array(
                    'Subdivision_ID' => $compare_subdivision_id,
                    'Sub_Class_Name' => NETCAT_MODULE_NETSHOP_COMPARE_SUBDIVISION_NAME,
                    'EnglishName' => 'compare',
                ));
            } else {
                $compare_subdivision_id = 0;
            }
        }

        $favorites_subdivision_id = $nc_core->catalogue->get_by_id($site_id, 'Netshop_Favorites_Sub_ID');
        // раздела "Избранные товары" еще нет
        if (!$favorites_subdivision_id) {
            // раздел "Избранные товары"
            $favorites_component_id = $this->get_special_component_id(self::NETSHOP_FAVORITES_COMPONENT_KEYWORD);
            if ($favorites_component_id) {
                $favorites_subdivision_id = $nc_core->subdivision->create(array(
                    'Catalogue_ID' => $site_id,
                    'Subdivision_Name' => NETCAT_MODULE_NETSHOP_FAVORITES_SUBDIVISION_NAME,
                    'EnglishName' => 'favorites',
                    'Checked' => 0,
                ));

                $nc_core->sub_class->create($favorites_component_id, array(
                    'Subdivision_ID' => $favorites_subdivision_id,
                    'Sub_Class_Name' => NETCAT_MODULE_NETSHOP_FAVORITES_SUBDIVISION_NAME,
                    'EnglishName' => 'favorites',
                ));
            } else {
                $favorites_subdivision_id = 0;
            }
        }

        $my_orders_subdivision_id = $nc_core->catalogue->get_by_id($site_id, 'Netshop_Order_List_Sub_ID');
        // раздела "Мои заказы" еще нет
        if (!$my_orders_subdivision_id) {
            // раздел "Мои заказы"
            $profile_subdivision_id = $nc_core->catalogue->get_by_id($site_id, 'Auth_Cabinet_Sub_ID',true);
            $order_list_template_component_id = $nc_core->component->get_component_template_by_keyword($order_component_id, self::NETSHOP_ORDER_LIST_TEMPLATE_KEYWORD, 'Class_ID');
            if ($profile_subdivision_id) {
                $my_orders_subdivision_id = $nc_core->subdivision->create(array(
                    'Catalogue_ID' => $site_id,
                    'Parent_Sub_ID' => $profile_subdivision_id,
                    'Subdivision_Name' => NETCAT_MODULE_NETSHOP_ORDER_LIST_SUBDIVISION_NAME,
                    'EnglishName' => 'orders',
                    'Checked' => 1,
                ));
                if ($order_list_template_component_id) {
                    $nc_core->sub_class->create($order_component_id, array(
                        'Subdivision_ID' => $my_orders_subdivision_id,
                        'Class_Template_ID' => $order_list_template_component_id,
                        'Sub_Class_Name' => NETCAT_MODULE_NETSHOP_ORDER_LIST_SUBDIVISION_NAME,
                        'EnglishName' => 'order',
                    ));
                }
            } else {
                $my_orders_subdivision_id = 0;
            }
        }

        $delivery_subdivision_id = $nc_core->catalogue->get_by_id($site_id, 'Netshop_Delivery_Sub_ID');
        // раздела "Условия доставки и возврата" еще нет
        if (!$delivery_subdivision_id) {
            // Раздел "Условия доставки и возврата"
            $delivery_subdivision_id = $nc_core->subdivision->create(array(
                'Catalogue_ID' => $site_id,
                'Subdivision_Name' => NETCAT_MODULE_NETSHOP_DELIVERY_SUBDIVISION_NAME,
        ...
        nc_db_table::make('Catalogue')->where_id($site_id)->update(array(
            'Netshop_Cart_Sub_ID' => $cart_subdivision_id,
            'Netshop_Order_Success_Sub_ID' => $success_subdivision_id,
            'Netshop_Order_List_Sub_ID' => $my_orders_subdivision_id,
            'Netshop_Compare_Sub_ID' => $compare_subdivision_id,
            'Netshop_Favorites_Sub_ID' => $favorites_subdivision_id,
            'Netshop_Delivery_Sub_ID' => $delivery_subdivision_id
        ));
    }
