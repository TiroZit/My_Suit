...
<input type="hidden" name="matching[item_key]" value="<?= $item_key; ?>">
...
<div style="display:<?= $object['format'] == nc_netshop_exchange_import::FORMAT_PRICE ? 'none' : null; ?>">
    ...
    <label><input type="radio" name="matching[subdivision_source]" value="current" <?= $subdivision_source == 'current' ? 'checked' : null; ?>> <?= NETCAT_MODULE_NETSHOP_EXCHANGE_SELECT_SUBDIVISION_THAT_EXISTS; ?></label><br>
    <label><input type="radio" name="matching[subdivision_source]" value="new" <?= $subdivision_source == 'new' ? 'checked' : null; ?>> <?= NETCAT_MODULE_NETSHOP_EXCHANGE_CREATE_SUBDIVISION; ?></label>
    ...
    <div class="nc-margin-top-medium">
        ...
        <div id="nc-netshop-exchange-subdivision-new" style="display: <?= $subdivision_source == 'current' ? 'none' : null; ?>">
            <div class="nc-field nc-field-type-string">
                ...
                <input name="matching[subdivision_name]" type="text" maxlength="255" size="50" value="<?= $new_subdivision_possible_name; ?>">
            </div>

            <div class="nc-field nc-field-type-string">
                <span class="nc-field-caption"><?= NETCAT_MODULE_NETSHOP_EXCHANGE_NEW_SUBDIVISION_KEYWORD; ?>:</span>
                <input name="matching[subdivision_english_name]" type="text" value="<?= $new_subdivision_possible_keyword; ?>" size="50" class="input" data-type="transliterate" data-from="matching[subdivision_name]" data-is-url="yes">
            </div>
        </div>
    </div>
...
<?php if (!$has_goods) { ?>
    ...
    <input type="hidden" name="matching[only_subdivision]" value="1">
...
<?php } else { ?>
    ...
    <div style="display:<?= $object['format'] == nc_netshop_exchange_import::FORMAT_PRICE ? 'none' : null; ?>">
        ...
        <label><input type="radio" name="matching[component_source]" value="current"> <?= NETCAT_MODULE_NETSHOP_EXCHANGE_SELECT_COMPONENT_THAT_EXISTS; ?></label><br>
        <label><input type="radio" name="matching[component_source]" value="new"> <?= NETCAT_MODULE_NETSHOP_EXCHANGE_CREATE_COMPONENT_BASED_ON_ANOTHER; ?></label>
        ...
        <div class="nc-field nc-margin-top-medium">
            ...
            <select name="matching[component_id]">
        ...
        <div id="nc-netshop-exchange-component-new">
            <div class="nc-field nc-field-type-string" id="nc-netshop-exchange-component-name">
                ...
                <input name="matching[component_name]" type="text" maxlength="255" size="50" value="<?= NETCAT_MODULE_NETSHOP_EXCHANGE_GOOD; ?>: <?= $new_component_possible_name; ?>">
            </div>
            <div class="nc-field nc-field-type-string" id="nc-netshop-exchange-component-keyword">
                ...
                <input name="matching[component_keyword]" type="text" maxlength="255" size="50" value="<?= $new_component_possible_keyword; ?>">
            </div>
        </div>
    </div>
    ...
    <h4 class="nc-margin-top-medium" style="display: inline-block; font-weight: bold;"><?= NETCAT_MODULE_NETSHOP_EXCHANGE_MATCHING_FIELDS; ?></h4>

    <div style="display: <?= $show_skip_rows ? 'block' : 'none'; ?>">
        <?= NETCAT_MODULE_NETSHOP_EXCHANGE_SKIP_FIRST; ?>
        <input type="number"
               name="matching[skip_rows]"
               value="<?= $skip_rows; ?>"
    ...
    <table class="nc-table nc--bordered nc--wide">
        ...
        <?php for ($i = 0; $i < count($preview[0]); $i++) {
            ...
            <tr>
                <td><?= ($i + 1); ?></td>
                <td class="nc--nowrap">
                    <select name="matching[field_<?= $i; ?>]" class="nc-netshop-exchange-field-select" data-id="<?= $i; ?>" data-new="<?= (int)$is_new_field; ?>"></select>
                    <button type="button" class="nc-netshop-exchange-btn-edit-field" data-id="<?= $i; ?>"><i class="nc-icon nc--settings"></i></button>
                    <input type="hidden" name="matching[field_<?= $i; ?>_key]" class="nc-netshop-exchange-new-field-input nc-netshop-exchange-new-field-input__key" value="<?= $fields_keys[$i]; ?>">
                    <input type="hidden" name="matching[field_<?= $i; ?>_name]" class="nc-netshop-exchange-new-field-input nc-netshop-exchange-new-field-input__name" value="<?= $new_field_name; ?>" data-id="<?= $i; ?>">
                    <input type="hidden" name="matching[field_<?= $i; ?>_description]" class="nc-netshop-exchange-new-field-input nc-netshop-exchange-new-field-input__description" value="<?= $new_field_description; ?>" data-id="<?= $i; ?>">
                    <input type="hidden" name="matching[field_<?= $i; ?>_type]" class="nc-netshop-exchange-new-field-input nc-netshop-exchange-new-field-input__type" value="<?= NC_FIELDTYPE_STRING; ?>" data-id="<?= $i; ?>">
                    <input type="hidden" name="matching[field_<?= $i; ?>_format]" class="nc-netshop-exchange-new-field-input nc-netshop-exchange-new-field-input__format" data-id="<?= $i; ?>">
                </td>
...
(function() {
    ...
    window.nc_netshop_exchange_wizard_form_submit = function() {
        ...
        var subdivisionSource = $nc('input[name="matching[subdivision_source]"]:checked').val();
        ...
        if (subdivisionSource === 'new') {
            ...
            data.subdivision_base = $nc('select[name="matching[subdivision_id]"]').val();
            data.subdivision_name = $nc('input[name="matching[subdivision_name]"]').val();
            data.subdivision_keyword = $nc('input[name="matching[subdivision_english_name]"]').val();
        }

        <?php if ($has_goods) { ?>

            // Использование существующего компонента или создание нового?
            var componentSource = $nc('input[name="matching[component_source]"]:checked').val();
            ...
            if (componentSource === 'new') {
                ...
                data.component_base = $nc('select[name="matching[component_id]"] option:selected').val();
                data.component_name = $nc('input[name="matching[component_name]"]').val();
                data.component_keyword = $nc('input[name="matching[component_keyword]"]').val();
                ...
                $nc(document).find('.nc-netshop-exchange-field-select').each(function() {
                    ...
                    // Ключ поля
                    var fieldKey = selectForField.attr('name').replace('matching[', '').replace(']', '');
                    var fieldIndex = parseInt(fieldKey.replace('field_', '')) + 1;

                    var parent = selectForField.parent().parent();

                    var fieldName = parent.find('input[name="matching[' + fieldKey + '_name]"]').val();
                    var fieldDescription = parent.find('input[name="matching[' + fieldKey + '_description]"]').val();
                    var fieldType = parseInt(parent.find('input[name="matching[' + fieldKey + '_type]"]').val());
                    var fieldFormat = parent.find('input[name="matching[' + fieldKey + '_format]"]').val();
            ...
            if (requiredFields.length) {
                var str = '<?= NETCAT_MODULE_NETSHOP_EXCHANGE_REQUIRED_FIELDS_NOT_MATCHED; ?>: ' + requiredFieldsInfo.join(', ') + '!';
                errors.push(str);
            }
        ...
        $nc.ajax({
            url: '<?= nc_module_path(); ?>netshop/admin/?controller=exchange&action=check_matching_data',
            type: 'POST',
            data: data,
    ...
    <?php if ($has_goods) { ?>
        ...
        var components = null;
        var selectedFields = <?= nc_array_json(!empty($matching['fields']) ? $matching['fields'] : array()); ?>;
                    ...
                    refreshFieldsNewField(function() {
                        modalProgressSet(++counter, progressTotal, function() {
                            refreshFieldsHideAlreadySelectedFields(function() {
                                modalProgressSet(++counter, progressTotal, function() {
                                    refreshFieldsMapNotMatchedFields(function() {
                                        modalProgressSet(++counter, progressTotal, function() {
                                            setTimeout(function() {
        ...
        function refreshFieldsMapNotMatchedFields(callback) {
            <?php $fields_can_be_hard_matched = $object->get_hard_matched_fields();
                if (!empty($fields_can_be_hard_matched)) { ?>

                    var convertFields = <?= nc_array_json(!empty($fields_can_be_hard_matched) ? $fields_can_be_hard_matched : array()); ?>;
        ...
        function refreshFieldsNewField(callback) {
            var value = $nc(document).find('input[name="matching[component_source]"]:checked').val();
        ...
        function updateComponentBySubdivision() {
            var subdivisionSource = $nc(document).find('input[name="matching[subdivision_source]"]:checked').val();
            if (subdivisionSource !== 'current') {
                return;
            }
            var subdivisionId = $nc(document).find('select[name="matching[subdivision_id]"] option:selected').val();
            var data = {
                nc_token: '<?= $nc_core->token->get(); ?>',
            ...
            $nc.ajax({
                ...
                success: function(response) {
                    ...
                    var currentComponentSource = $nc(document).find('input[name="matching[component_source]"]:checked').val();
                    var currentComponentId = parseInt($nc(document).find('select[name="matching[component_id]"]').val());
                    if (currentComponentSource === 'current' && currentComponentId === componentId) {
                        return;
                    }
                    $nc('select[name="matching[component_id]"] option').removeAttr('selected');
                    $nc('select[name="matching[component_id]"] option[value="' + componentId + '"]').prop('selected', true);
                    $nc('input[name="matching[component_source]"]').removeAttr('checked');
                    $nc('input[name="matching[component_source]"][value="current"]').prop('checked', true);
                    $nc('input[name="matching[component_source]"]').trigger('change');
                }
            });
        }
        ...
        $nc(function() {
            ...
            $nc(document).on('change', 'select[name="matching[component_id]"]', function() {
                var componentId = parseInt($nc(this).val());

                refreshFieldsSelects(componentId, selectedFields);
            });

            $nc(document).on('change', 'input[name="matching[component_source]"]', function() {
                switch ($nc('input[name="matching[component_source]"]:checked').val()) {
                    case 'current':
                ...
                refreshFieldsSelects(parseInt($nc('select[name="matching[component_id]"]').val()), selectedFields);
            });
            ...
            setTimeout(function() {
                if (selectedComponentId !== null) {
                    $nc(document).find('select[name="matching[component_id]"]').val(selectedComponentId).trigger('change');
                    $nc(document).find('input[name="matching[component_source]"][value="current"]').trigger('click');
                } else {
                    <?php $component_keys = array_keys($components); ?>
                    refreshFieldsSelects(<?= $component_keys[0]; ?>, selectedFields);
                    $nc(document).find('input[name="matching[component_source]"][value="new"]').trigger('click');
                }
            }, 1000);
        });
    ...
    $nc(function() {
        $nc(document).on('change', 'input[name="matching[subdivision_source]"]', function() {
            switch ($nc(this).val()) {
                case 'current': {
        ...
        $nc(document).on('change', 'select[name="matching[subdivision_id]"]', function() {
            updateComponentBySubdivision();
        });
        ...
        setTimeout(function() {
            $nc(document).find('input[name="matching[subdivision_source]"][value="current"]').trigger('click');
        }, 200);
