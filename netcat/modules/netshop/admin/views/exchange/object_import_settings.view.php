...
/** @var boolean $saved */
/** @var integer $cron_minutes */
/** @var integer $cron_hours */
/** @var integer $cron_days */
...
<?php if ($object->get_type() == nc_netshop_exchange_object::TYPE_IMPORT) { ?>
    <?php if ($object->get_status() == nc_netshop_exchange_handler_import::STATUS_IMPORTING) { ?>

        <div class="nc-alert nc--blue">
            <i class="nc-icon-l nc--status-info"></i>
            <?= sprintf(NETCAT_MODULE_NETSHOP_EXCHANGE_INFO_IMPORTING_RIGHT_NOW, $object->handler->get_percent(true)); ?>
        </div>

        <?php if ($object->exchange_may_be_stuck()) { ?>

            <div class="nc-alert nc--yellow">
                <i class="nc-icon-l nc--status-warning"></i>
                <?= NETCAT_MODULE_NETSHOP_EXCHANGE_INFO_EXCHANGE_STUCK; ?>
                <br>
                <a href="<?= nc_get_http_folder($ADMIN_PATH); ?>#module.netshop.exchange.import(reset_status,<?= $object->get('exchange_id'); ?>)"
                   target="_top"
                   class="nc-btn nc--mini nc--blue">
                    <?= NETCAT_MODULE_NETSHOP_EXCHANGE_RESET_STATUS; ?>
                </a>
            </div>
