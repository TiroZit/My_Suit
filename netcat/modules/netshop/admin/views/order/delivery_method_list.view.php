    ...
    (function() {
        function init_map() {
            ...
            options.map_point_selection_callback = function (delivery_point_id) {
                var delivery_point_radio = $nc('.nc-netshop-order-delivery-point-radio[value="' + delivery_point_id +'"]'),
            ...
            $nc('.nc-netshop-order-delivery-type-pickup-map-button')
                ...
                .on(click_event, function() {
                    if (map.is_ready) {
                        map.get_map().options.set('fullscreenZIndex', 10013);
...
.nc-netshop-order-delivery-list .nc-netshop-order-delivery-type button:hover {
    padding: 1px 5px;
    margin-left: 20px;
}
