    ...
    <div class="nc-dashboard-grid">
        <? foreach ($order_status_counts as $status_id => $status): ?>
            <div class="nc-widget nc--lighten">
                <table class="nc-widget-grid nc-widget-link" onclick="window.location='<?= nc_module_path('netshop'); ?>admin/?controller=order&amp;action=index&amp;site_id=<?= $catalogue_id ?>&amp;order_status=<?=$status_id ?>'">
                    <tr>
                        <td>
