...
i;m++){var n=c.conditions[m];"and"===n.type||"or"===n.type?this.addGroup(d,n):this.addCondition(n.type,d,n)}g&&1===i&&!("and"===c.conditions[0].type||"or"===c.conditions[0].type)&&k.find(".operator-cell").hide()}this.updateOperatorLabelPosition(h.first())};d.showAddConditionDialog=function(a){var c=["<option></option>","<option value='addGroup' class='create-group-option'>"+b.ADD_GROUP+"</option>"],e=null,k;for(k in this.conditionTypes){var d=this.conditionTypes[k],h=this.conditionGroupsToShow;if(!(h.length&&
-1==f.inArray(d.group,h))&&-1==f.inArray(d.group,this.conditionGroupsToExclude)&&-1==f.inArray(k,this.conditionsToExclude)){if(e!=d.group)null!=e&&c.push("</optgroup>"),c.push('<optgroup label="'+b[d.group]+'">'),e=d.group;c.push('<option value="'+k+'">'+b[d.label]+"</option>")}}c.push("</optgroup>");var g=f('<select data-placeholder="'+b.SELECT_CONDITION_TYPE+'">'+c.join("")+"</select>"),i=f('<div class="add-condition-type-select"></div>').append(g).appendTo(a),j=this;g.chosen(this.defaultChosenParams).on("chosen:hiding_dropdown",
function(){var a=g.val(),c=g.closest(".condition-cell");g.parents(".add-condition-type-select").remove();"addGroup"==a?j.addGroup(c,null):a&&j.addCondition(a,c,{})}).on("chosen:showing_dropdown",t);setTimeout(function(){i.find(".chosen-container").trigger("mousedown")},1)};d.updateAllConditionLabelPositions=function(){var a=this;this.root.find(".condition-group").each(function(){a.updateOperatorLabelPosition(f(this))})};d.updateOperatorLabelPosition=function(a){if(!this.isInitializing){var c=a.find(".condition-cell").first().children(".condition, .condition-group"),
