...
do {
    ...
    require_once nc_module_folder('netshop') . 'old/xml.lib.php';
            ...
            $res = $db->get_results(
                    "SELECT Subdivision_ID, Class_ID, Sub_Class_ID
                      ...
                      WHERE Subdivision_ID IN (" . join(", ", $sub_ids) . ")
                        AND Class_ID IN (" . join(", ", array_keys($goods_classes)) . ")
                      ORDER BY Subdivision_ID, Priority",
                    ARRAY_N
                );
            foreach ($res as $row) {
                list($sub_id, $class_id, $subclass_id) = $row;
                if (!isset($templates[$sub_id])) {
...
//   $count = $count->value;
    ...
    if ($everything_clear) {
        ...
        foreach ($class_fields[$this_class] as $property_id => $property) {
            ...
            //else {
            $percent = sprintf("%.01f", ($current_num / $count * 100));
    ...
    if ($current_num > $count && $everything_clear) {
        ...
        if (!$silent_1c_import) {
            ...
            printf(NETCAT_MODULE_NETSHOP_IMPORT_1C_LINK, nc_get_scheme() . '://' . $HTTP_HOST . nc_module_path('netshop') . "import/1c.php?source_id=$source_id&key=" .
