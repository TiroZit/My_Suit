...
if ($_GET['mode'] == 'file') {
    ...
    if ($_GET['type'] == 'sale' && !$intercept) {
        ...
        require_once nc_module_folder('netshop') . 'import/nc_netshop_cml2parser.class.php';
...
if ($_GET['mode'] === 'query' && $_GET['type'] === 'sale') {
    ...
    // make user's undivine
    @ignore_user_abort(true);
    include_once nc_module_folder('netshop') . 'function.inc.php';
    ...
    if (is_numeric($export_orders_status) || !empty($export_orders_status)) {
        $where_status = "AND m.`Status` IN (" . $nc_core->db->escape($export_orders_status) . ")";
    } else {
        $where_status = '';
    }
    ...
    $add_payments = nc_module_check_by_keyword('payment', false);

    echo '<?xml version="1.0"  encoding="windows-1251"?>' . PHP_EOL;
    echo '<КоммерческаяИнформация ВерсияСхемы="2.09" ДатаФормирования="' . date("Y-m-d") . '">' . PHP_EOL;
    ...
    foreach ($orders_arr as $order_id) {
        ...
        if ($add_payments) {
            $paid_invoices = $db->get_results(
                "SELECT `Payment_Invoice_ID`, 
                        DATE_FORMAT(`Created`, '%Y-%m-%dT%H:%i:%s') AS `Date`,
                        `Amount`,
                        `Payment_System_ID`
                   FROM `Payment_Invoice` 
                  WHERE `Order_Source` = 'netshop' 
                    AND `Order_ID` = $order_id
                    AND `Status` = '" . nc_payment_invoice::STATUS_SUCCESS . "'",
                ARRAY_A
            );
            ...
            if ($paid_invoices) {
                ...
                echo '    <Оплаты>' . PHP_EOL;
                foreach ($paid_invoices as $invoice_data) {
                    $payment_method_name = $payment_method
                        ? $payment_method->get('name')
                        : nc_get_list_item_name('PaymentSystem', $invoice_data['Payment_System_ID']);
                    echo '        <Оплата>' . PHP_EOL;
                    $spaces = str_repeat(' ', 12);
                    echo "$spaces<НомерДокумента>$invoice_data[Payment_Invoice_ID]</НомерДокумента>" . PHP_EOL,
                         "$spaces<НомерТранзакции>$invoice_data[Payment_Invoice_ID]</НомерТранзакции>" . PHP_EOL,
                         "$spaces<ДатаОплаты>$invoice_data[Date]</ДатаОплаты>" . PHP_EOL,
                         "$spaces<СуммаОплаты>$invoice_data[Amount]</СуммаОплаты>" . PHP_EOL,
                         "$spaces<СпособОплаты>" . xmlspecialchars($payment_method_name) . "</СпособОплаты>" . PHP_EOL,
                         "$spaces<ИдСпособаОплаты>$order[PaymentMethod]</ИдСпособаОплаты>" . PHP_EOL;
                    echo '        </Оплата>';
                }
                echo '    </Оплаты>';
            }
...
if ($_GET['mode'] == 'query' && $_GET['type'] == 'get_catalog') {
    ...
    if ($step == 1) {
        ...
        require_once nc_module_folder('netshop') . 'export/cml2_catalog.inc.php';
