...
class nc_netshop_cml2parser {
    ...
    function nc_netshop_cml2parser(&$db, &$_UTFConverter, $source_id, $catalogue_id, $filename, $quite = false) {
        ...
        if ($this->nc_core->NC_UNICODE) {
            require_once nc_module_folder('netshop') . 'ru_utf8.lang.php';
        } else {
            require_once nc_module_folder('netshop') . 'ru_cp1251.lang.php';
        }
        ...
        $this->filedir = realpath($GLOBALS['TMP_FOLDER'] . $path_info['dirname']) . '/';
        $this->filename = $path_info['basename'];
        ...
        // cml2 class
        require_once nc_module_folder('netshop') . 'import/cml2.class.php';
        $this->cml2 = new cml2();
    ...
    function cache_data_destroy($suffix = "") {
        ...
        if ($handle = opendir($this->filedir)) {
            ...
            while (($file = readdir($handle)) !== false) {
                // determine file
                if (preg_match("/^" . preg_quote($this->filename, '/') . "\." . $suffix . "/is", $file)) {
                    // file path
    ...
    function import_classifier_data() {
        ...
        foreach ($classifier_data as $key => $value) {
                ...
                default:
                    ...
                    if ($sub_id != -1) {
                        ...
                        if (!$sub_id) {
                            ...
                            // ID инфоблока ($sub_class_id) в методе import_catalogue_data() будет взято
                            // из $this->get_templates(), нет необходимости загружать здесь ещё раз
            ...
            if ($group_already_saved && $parent_gid && $auto_move_sections) {
                $multiple_subdivisions_mapped = $this->db->get_var("SELECT COUNT(*) FROM `Netshop_ImportMap` WHERE `type` = 'section' AND `value` = $sub_id") > 1;

                if ($multiple_subdivisions_mapped) {
                    // разделу в Netcat сопоставлено несколько групп 1C — не трогаем такой раздел
                    $parent_group_sub_id = false;
                } else {
    ...
    function import_offers_data() {
        ...
        foreach ($offers_data as $offer) {
            if (!empty($offer[NETCAT_MODULE_NETSHOP_1C_PRICES])) {
        ...
        while ($offers_data_offers = $this->cache_data_out("offers_data_offers" . $i)) {
            ...
            foreach ($offers_data_offers as $offer_data) {
                ...
                $this_component = $nc_core->get_component($this_class);
                ...
                foreach ($packets AS $packet_id => $packet) {
                    ...
                    $this_prop["Currency" . $currency_add] = $this_currency;
                ...
                if (isset($offer_data[NETCAT_MODULE_NETSHOP_1C_STORE_REMAIN])) {
                    ...
                    foreach ($stores_qty as $store) {
                        ...
                        $qty = $store[NETCAT_MODULE_NETSHOP_1C_STORE_QTY] ?: '0';
                ...
                if (!empty($this_prop)) {
                    ...
                    foreach ($this_prop AS $k => $v) {
                        if ($k) {
                            if ($this_component->get_field($k, 'type') == NC_FIELDTYPE_SELECT) {
                                $v = $this->get_list_item_id($this_component->get_field($k, 'table'), $v);
                            }
                ...
                if ($offer_is_variant || isset($offer_data[NETCAT_MODULE_NETSHOP_1C_PRODUCT_CHARS])) {
                    ...
                    foreach($mapped_characteristics[$this_class] as $key => $value) {
                        ...
                        if ($field_name) {
                            ...
                            if ($characteristics && isset($characteristics[$key])) {
                                ...
                                $characteristics_values[$field_name] = $characteristics[$key];
                            }
                        }
                    }
                    ...
                    // МойСклад в качестве названия вариантов передаёт название основного товара.
                    // Чтобы название вариантов отличалось, пробуем слепить название из того, что есть.
                    // В 1С в качестве ID используется UUID, в МС — строка из символов \w+
                    $fill_variant_name =
                        $offer_is_variant &&
                        !preg_match('/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}/', $offer_1c_id) &&
                        $this_component->has_field('Name') &&
                        $this_component->has_field('VariantName') &&
                        empty($variant_fields['VariantName']);
                    if ($fill_variant_name) {
                        $variant_fields['VariantName'] .= '(' . trim(implode(', ', $characteristics_values)) . ')';
                    }
                    ...
                    foreach ($variant_fields as $field => $characteristic) {
                        if ($this_component->get_field($field, 'type') == NC_FIELDTYPE_SELECT) {
                            $characteristic = $this->get_list_item_id($this_component->get_field($field, 'table'), $characteristic);
                        }
    ...
    function import_catalogue_data() {
        ...
        if (count($classificator_fields)) {
            $clformats = $this->db->get_results("SELECT `Field_ID`,`Format` from `Field` WHERE `Field_ID` IN (" . join(",", $classificator_fields) . ") AND `TypeOfData_ID`=" . NC_FIELDTYPE_SELECT, ARRAY_A);
        ...
        while ($catalogue_data_commodity = $this->cache_data_out("catalogue_data_commodity" . $i)) {
            ...
            foreach ($catalogue_data_commodity as $key => $value) {
                ...
                if ($this_prop["Subdivision_ID"] == -1 || !$this_class) {
                    // increment
                    $current_num++;
                    continue;
                }
          ...
          AND `ImportSourceID` = '" . $this->source_id . "'");
                ...
                // массив со списком добавленных файлов (на случай, если Multifield содержит записи об отсутствующих на диске файлах)
                $files_to_keep = array();

                if (!empty($class_fields) && !empty($fields) && $this_class)
                    foreach ($class_fields[$this_class] AS $field_key => $field_data) {
                        if ($field_data['column'] == -1) {
                            continue;
                        }

                        $field_settings = $this_component->get_field($field_data['column']);
                        if (!$field_settings) {
                            continue;
                        }
                        ...
                        $field_format = $field_settings['format'];
                        ...
                        switch (true) {
                            ...
                            case is_array($this_tax) && array_key_exists($field_key, $this_tax):
                                ...
                                break;
                            // тэг "Картинка"
                            case $field_type == NC_FIELDTYPE_FILE && $field_key == NETCAT_MODULE_NETSHOP_1C_IMG:
                                ...
                                break;
                            case $field_type == NC_FIELDTYPE_MULTIFILE && $field_key == NETCAT_MODULE_NETSHOP_1C_IMG:
                                /** filename similarly commodity ID ($this_id) */
                                $this_prop[$field_name] = '';
                                ...
                                foreach ($value[$field_key] as $j => $xml_value) {
                                    ...
                                    $http_path = $nc_core->HTTP_FILES_PATH;
                                    $path = nc_standardize_path_to_folder($http_path . '/' . $file_copy_path) . $uniq_file_name;
                                    $files_to_keep[$path] = true;
                                        ...
                                        if ($paths) {
                                            foreach ($paths as $item) {
                                                foreach (array('Path', 'Preview') as $key) {
                                                    if (!isset($files_to_keep[$item[$key]])) {
                        ...
                        if ($field_name != 'Units' && $field_type == NC_FIELDTYPE_SELECT && isset($this_prop[$field_name])) {
                            $this_prop[$field_name] = $this->get_list_item_id($field_settings['table'], $this_prop[$field_name]);
                        }
    ...
    /**
     * Возвращает идентификатор элемента списка по его значению
     *
     * @param string $classificator_name
     * @param string $value
     * @return int
     */
    protected function get_list_item_id($classificator_name, $value) {
        $classificator_id = 0;
        if (strlen($value)) {
            $classificator_value = $this->db->escape($value);

            $classificator_id = $this->db->get_var(
                "SELECT `{$classificator_name}_ID` 
                   FROM `Classificator_{$classificator_name}` 
                 WHERE `{$classificator_name}_Name` = '{$classificator_value}'"
            );

            if (!$classificator_id) {
                $new_priority = 1 + $this->db->get_var(
                    "SELECT MAX(`{$classificator_name}_Priority`) AS `Priority` 
                       FROM `Classificator_{$classificator_name}`"
                );

                $this->db->query(
                    "INSERT INTO `Classificator_{$classificator_name}` 
                        SET `{$classificator_name}_Name` = '{$classificator_value}', 
                            `{$classificator_name}_Priority` = '{$new_priority}'"
                );
                $classificator_id = (int)$this->db->insert_id;
            }
        }

        return $classificator_id;
    }
