...
require_once $nc_core->ADMIN_FOLDER . 'lang/Russian.php'; // нужна nc_transliterate()
...
// Максимальный размер передаваемого файла в байтах (не более 5 Мб)
$max_file_size = min(5 * 1024 * 1024, nc_netshop_exchange_helper::parse_size(ini_get('post_max_size')));
...
try {
    $exchange_object = nc_netshop_exchange_import_cml::by_id($exchange_object_id);
} catch (Exception $e) {
    ...
    case 'catalog': {
        ...
        switch ($mode) {
            ...
            case 'file': {
                ...
                $file_data = $importer->save_to_file($file_name, $file_content);
                if ($file_data['error']) {
                    log_to_file("Выгрузка каталога товаров : Получение файлов : Файл {$file_name} не записан (путь: {$file_data['path']})");
                    echo "Unable to write to {$file_data['path']}";
                } else {
