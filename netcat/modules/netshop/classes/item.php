...
class nc_netshop_item implements ArrayAccess {
    ...
    protected function get_url() {
        try {
            ...
            return nc_core::get_object()->catalogue->get_url_by_id($this['Catalogue_ID']) . $path;
        } catch (Exception $e) {
            return false;
        }
    }
        ...
        if (!isset(self::$component_fields[$component_id])) {
            ...
            foreach ($component->get_fields() as $field) {
                ...
                if ($type == NC_FIELDTYPE_SELECT) {
                    $extra_subfields = $special_subfields = array("id", "name", "value", "settings");
                    self::$special_component_fields[$component_id][$name] = $field;
                }
                elseif ($type == NC_FIELDTYPE_MULTISELECT) {
                    $extra_subfields = $special_subfields = array("id", "value", "settings");
                    self::$special_component_fields[$component_id][$name] = $field;
                }
    ...
    protected function set_special_field_value($property) {
        ...
        elseif ($field['type'] == NC_FIELDTYPE_SELECT) {
            $this->set_select_fields_values($field);
        }
    }
     ...
     * @param array $field
     */
    protected function set_select_fields_values($field) {
        $nc_core = nc_Core::get_object();
        $custom_settings = $this->additional_data[$field['name'] . '_settings'] ?: $this->item_data[$field['name'] . '_settings'];
        $this->offsetSet("$field[name]_settings", $nc_core->list->evaluate_item_settings($field['table'], $custom_settings));

        $this->uninitialized_special_fields["$field[name]_settings"] = null;
    }
    ...
    protected function set_multiselect_fields_values($field) {
        $names = $ids = $values = $settings = array();

        // set all fields at once: <field>, <field>_id, <field>_value, <field>_settings
        ...
        if (!$raw_value && $this->has_parent()) {
            ...
            $this->offsetSet($f . "_settings", $parent[$f . "_settings"]);
            return;
        }
        ...
        if (preg_match('/^[\d,]+$/', $raw_value)) { // looks not harmful
            ...
            $query = "SELECT `{$t}_Name`, `{$t}_ID`, `Value`, `CustomSettings`
            ...
            foreach ((array)$result as $row) {
                ...
                $ids[] = $row[1];
                $values[] = $row[2];
                $settings[] = $nc_core->list->evaluate_item_settings($t, $row[3]);
            }
        }
        ...
        $this->offsetSet("$field[name]_settings", $settings);
    ...
    public function offsetGet($property) {
        ...
        if ($this->is_property_defined($property)) {
            ...
            if ($property === 'VAT' && date('Y') >= 2019 && $value == 18) {
                $value = 20;
            }
