...
class nc_netshop_delivery_service_yandex_converter {
     ...
     */
    public function __construct(nc_netshop_delivery_service_yandex $service, array $delivery_data) {
    ...
    public function get_delivery_variants(array $response) {
        ...
        foreach (nc_array_value($response, 'data', array()) as $data) {
            ...
            $variant = new nc_netshop_delivery_method_variant(array(
                ...
                'extra_charge_absolute' => $data['costWithRules'],
                'minimum_delivery_days' => $data['minDays'],
                'maximum_delivery_days' => $data['maxDays'],
                '_yandex_delivery_id' => $data['delivery']['id'],
                '_yandex_to_yd_warehouse' => $data['settings']['to_yd_warehouse'],
            ));
