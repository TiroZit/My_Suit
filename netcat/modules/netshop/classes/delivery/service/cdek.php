...
class nc_netshop_delivery_service_cdek extends nc_netshop_delivery_service {
    ...
    const CDEK_CALCULATE_URL = 'https://api.cdek.ru/calculator/calculate_price_by_json.php';
    const CDEK_PICKUP_POINTS_URL = 'https://integration.cdek.ru/pvzlist.php';
    ...
    /** @var array обход особенностей СДЭК для некоторых городов */
    protected $cdek_postal_code_mapping = array(
        101000 => 'regionid=81', // Москва как город (в СДЭК в пределах МКАД) → Москва как регион
        190000 => 'regionid=82', // С.-Пб как город (в СДЭК в пределах МКАД) → С.-Пб как регион
        140961 => 'cityid=157', // Подольск МО
        450514 => 'cityid=657', // Октябрьский, Башкортостан
    );
    ...
    protected function get_common_calculation_request_data(nc_netshop_delivery_method $method) {
        ...
        $request_data = array(
            ...
            'senderCity' => (string)$this->data['from_city'],
            'receiverCity' => (string)$this->data['to_city'],
            'goods' => array(),
        );
        ...
        foreach ($items as $item) {
            ...
            for ($i = 0; $i < $item['Qty']; $i++) {
                $request_data['goods'][] = $item_request_data;
            }
        }
    ...
    public function get_cdek_delivery_points_by_postal_code($postal_code, $total_weight_in_kg) {
        ...
        if (isset($this->cdek_postal_code_mapping[$postal_code])) {
            $query = $this->cdek_postal_code_mapping[$postal_code];
        } else {
            $query = 'countryiso=RU&citypostcode=' . $postal_code;
        }

        $response = $this->make_http_request(self::CDEK_PICKUP_POINTS_URL . '?' . $query);
        if (!$response) {
            return null;
        }
        ...
        $check_city_name = count($points_list->Pvz) > 500;
        ...
        foreach ($points_list->Pvz as $point_data) {
            ...
            // Для Москвы (например) RegionName = 'Москва', 'City' может быть 'Митино', 'Бутово', ...
            if ($check_city_name && (string)$point_data['City'] !== $this->data['to_city'] && (string)$point_data['RegionName'] !== $this->data['to_city']) {
                continue;
            }

            $description = '';
            if ((string)$point_data['Note']) {
                $description .= htmlspecialchars($point_data['Note']) . '<br>';
            }
            if ((string)$point_data['Site']) {
                $description .= "<a href='" . htmlspecialchars($point_data['Site'], ENT_QUOTES) . "' target='_blank'>" . NETCAT_MODULE_NETSHOP_DELIVERY_CDEK_POINT_LINK . '</a>';
            }
            ...
            $point_properties = array(
                ...
                'payment_on_delivery_cash' => ((string)$point_data['AllowedCod']) === 'true',
                'payment_on_delivery_card' => ((string)$point_data['HaveCashless']) === 'true',
                'enabled' => true,
            );
