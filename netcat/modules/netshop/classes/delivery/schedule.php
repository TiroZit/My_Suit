...
class nc_netshop_delivery_schedule extends nc_record_collection {
    ...
    public function get_compact_schedule_string() {
        ...
        foreach ($this->items as $item) {
            for ($day = 1; $day <= 7; $day++) {
                if ($item->get("day$day")) {
                    $day_schedules[$day][] = $item->get_time_interval_string();
                }
            }
        }

        // сортировка интервалов каждого дня по возрастанию (отдельно, до цикла «склеивания» интервалов)
        foreach ($day_schedules as $day => $schedule) {
            natsort($schedule);
            $day_schedules[$day] = implode(', ', $schedule);
        }
