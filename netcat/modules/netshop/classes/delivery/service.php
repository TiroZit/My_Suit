...
abstract class nc_netshop_delivery_service {
     ...
     *
     * @param string $url
     * @param string|array|null $data  тело POST (строка) или параметры для POST/GET (массив).
     *   Если не указано: будет выполнен GET-запрос, если $request_type === null или 'GET'
     *   Если $data — массив и заголовок Content-Type — application/json, будет преобразован с использованием json_encode().
     *   Если $data — массив и $request_type — GET, параметры будут дописаны к $url.
     * @param array $headers   массив с заголовками (заголовок => значение)
     * @param string $method тип запроса. Возможные значения: GET, POST, PUT, DELETE
     *   По умолчанию: POST, если задан $data; иначе GET
     * @return mixed|string
     */
    protected function make_http_request($url, $data = null, array $headers = array(), $method = null) {
        if (!$method) {
        ...
        if ($data) {
            if (is_array($data)) {
                ...
                } else if (nc_array_value($headers, 'Content-Type') === 'application/json') {
        ...
        if (!nc_array_value($headers, 'User-Agent')) {
            $headers['User-Agent'] = nc_array_value($_SERVER, 'HTTP_USER_AGENT', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.77 Safari/537.36');
        }

        if (function_exists('curl_init')) {
            return $this->fetch_with_curl($url, $data, $headers, $method);
        }

        return $this->fetch_with_stream($url, $data, $headers, $method);
    }
     ...
     *
     * @param string $url
     * @param string|array|null $body  тело POST
     * @param array $headers   массив с заголовками (заголовок => значение)
     * @param string $method тип запроса. Возможные значения: GET, POST, PUT, DELETE
     * @return mixed|string
     */
    protected function fetch_with_curl($url, $body, array $headers, $method) {
        $curl = curl_init($url);
     ...
     *
     * @param string $url
     * @param string|array|null $body  тело POST (строка) или параметры для POST (массив).
     *   Если не указано, будет выполнен GET-запрос.
     * @param array $headers   массив с заголовками (заголовок => значение)
     * @param string $method тип запроса. Возможные значения: GET, POST, PUT, DELETE
     * @return mixed|string
     */
    protected function fetch_with_stream($url, $body, array $headers, $method) {
