...
class nc_netshop_condition_order_deliverymethod extends nc_netshop_condition {
    ...
    public function evaluate(nc_netshop_condition_context $context, $current_item = null) {
        // при расчёте стоимости доставки ID способа доставки передаётся в $current_item
        if (is_numeric($current_item)) {
            return $this->compare($current_item, $this->op, $this->value);
        }

        // иначе условие проверяет способ доставки в свойствах заказа
        return $this->compare($context->get_order_property('DeliveryMethod'), $this->op, $this->value);
    }
        ...
        if (!isset($cache[$this->value])) {
            try {
                ...
                $cache[$this->value] = nc_ui_helper::get()->hash_link(
                    "module.netshop.delivery.method.edit($this->value)",
                    $method->get('name')
                );
            }
