...
class nc_netshop_location_provider_russianpost extends nc_netshop_location_provider {
    ...
    static public function get_first_postal_code(nc_netshop_location_data $location_data) {
        ...
        return nc_db()->get_var(
            "SELECT `Russianpost_Code`
              ...
              ORDER BY `Boost` DESC, `Russianpost_Code` LIKE '___000' DESC, `Russianpost_Code`
              LIMIT 1"
        );
    }
    ...
    public function find_locations($search_string) {
        ...
        foreach ($matches as $data) {
            $location_data = new nc_netshop_location_data($data);

            $is_exact_match =
                $data['name'] === $search_string ||
                ($data['uniqueness_level'] == 0 && $data['locality_name'] === $locality) ||
                ($data['uniqueness_level'] == 1 && $data['locality_name'] === $locality && $data['region_name'] === $region);
            $location_data->set('is_exact_match', $is_exact_match);

            $result->add($location_data);
        }
