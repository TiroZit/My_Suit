...
class nc_netshop_exchange_wizard_admin_controller extends nc_netshop_exchange_admin_controller {
    ...
    protected function init() {
        ...
        foreach (array('start', 'setup_automated_cml', 'selection', 'matching', 'config', 'report') as $action) {
        ...
        $this->bind('matching_save', array('object_id', 'phase'));
    }
    ...
    public function action_start_save($object_id = -1) {
        ...
        switch ($object['mode']) {
            ...
            case nc_netshop_exchange_object::MODE_MANUAL: {
                ...
                if (!empty($uploaded_files)) {
                    $uploaded = $object->folder->upload_files_from_request_data($uploaded_files, true);
                } else if (!empty($upload_file_url)) {
                    $uploaded = $object->folder->upload_file_from_url($upload_file_url, true);
        ...
        $this->redirect_to_index_action('matching', "&object_id={$object_id}&phase=1");
        return true;
    }
    ...
    public function action_selection($action, $site_id, $object_id = -1, $phase = 1) {
        ...
        // Объекты маппинга
        $object = nc_netshop_exchange_import::by_id($object_id);
        $items = $object->matching->get_all();
        $object_exchange_matching = json_decode($object->get('matching'), true);
        $has_checked_items = false;
        foreach ((array)$items as $i => $item_key) {
            $checked = !array_key_exists($item_key, $object_exchange_matching);
            $items[$i] = array(
                'checked' => $checked,
                'matched' => !$checked,
     ...
     * @return nc_ui_view
     */
    public function action_matching($action, $site_id, $object_id = -1, $phase = 1) {
        $view = $this->get_view($action, $site_id, $object_id, $phase);

        $view->step = NETCAT_MODULE_NETSHOP_EXCHANGE_STEP_MATCHING;
        $view->next_action = 'matching_save';
        ...
        $item_key = $object->matching->current();
        ...
        if ($has_goods) {
            ...
            $skip_rows = nc_array_value($object_exchange_matching[$item_key], 'skip_rows', $skip_rows);
        ...
        if (!empty($object_exchange_matching[$item_key]['subdivision_id'])) {
            $default_subdivision_id = $object_exchange_matching[$item_key]['subdivision_id'];
        ...
        if (empty($default_subdivision_id)) {
            ...
            if ($parent_subdivision_id_in_file) {
                foreach ($object_exchange_matching as $already_item_key => $already_data) {
        ...
        if (empty($default_subdivision_id)) {
            ...
            if (!empty($object_exchange_matching)) {
                foreach ($object_exchange_matching as $already_item_key => $already_data) {
                    $already_item_key_data = nc_netshop_exchange_helper::item_key_to_data($already_item_key);
        ...
        if (empty($default_subdivision_id)) {
            if (!empty($object_exchange_matching)) {
                $last_item_key = array_keys($object_exchange_matching);
                $last_item_key = $last_item_key[count($last_item_key) - 1];
                $last_item = $object_exchange_matching[$last_item_key];
        ...
        $view->subdivisions = nc_netshop_exchange_helper::print_tree_as_select($subdivisions, 'matching[subdivision_id]', $default_subdivision_id);
        ...
        $object->extend_hard_matched_fields($components, $object_exchange_matching, $data_captions);
        ...
        if (!empty($object_exchange_matching[$item_key]['component_id'])) {
            $default_component_id = $object_exchange_matching[$item_key]['component_id'];
            ...
            if (empty($default_component_id) && !empty($object_exchange_matching)) {
                foreach ($object_exchange_matching as $already_item_key => $already_data) {
                    $already_item_component_id = $already_data['component_id'];
                    if (!empty($already_item_component_id) && $already_item_component_id != $first_component_id) {
                        $default_component_id = $already_item_component_id;
        ...
        $view->matching = !empty($object_exchange_matching[$item_key]) ? $object_exchange_matching[$item_key] : null;
        $view->item_key = $item_key;
        $view->file_name = pathinfo($file_path, PATHINFO_BASENAME);
        $view->scope_name = $scope_name;
        $view->phases = $object->matching->total();
        $view->phase = $object->matching->index();
     ...
     * @param int $phase
     */
    public function action_matching_save($object_id = -1, $phase = 1) {
        $nc_core = nc_core::get_object();
        $object = nc_netshop_exchange_import::by_id($object_id);
        ...
        $matching_data = $this->input->fetch_post('matching');

        // Создание нового компонента
        if (!$matching_data['only_subdivision']) {
            if ($matching_data['component_source'] === 'new') {
                $fields = array();

                foreach ($matching_data as $key => $val) {
                ...
                // Создадим компонент
                $old_component_id = $matching_data['component_id'];
                $matching_data['component_id'] = $nc_core->component->create(array(
                    'Keyword' => nc_netshop_exchange_helper::get_component_possible_keyword($matching_data['component_keyword']),
                    'Class_Name' => $matching_data['component_name'],
                ), $matching_data['component_id']);

                // Создадим поля для него
                if (!empty($fields)) {
                    foreach ($fields as $field_index => $field) {
                        $matching_data['field_' . $field_index] = $nc_core->component->add_field($matching_data['component_id'], array(
                ...
                $field_index = 0;
                while (array_key_exists('field_' . $field_index, $matching_data)) {
                    // Id поля в базовом компоненте
                    $field_id = $matching_data['field_' . $field_index];
                    ...
                    $new_component_id = $matching_data['component_id'];
                    ...
                    $matching_data['field_' . $field_index] = $field_id;
        ...
        if ($matching_data['subdivision_source'] === 'new') {
            // Создадим раздел
            $matching_data['subdivision_id'] = $nc_core->subdivision->create(array(
                'Catalogue_ID' => $this->site_id,
                'Parent_Sub_ID' => $matching_data['subdivision_id'],
                'Subdivision_Name' => $matching_data['subdivision_name'],
                'EnglishName' => nc_netshop_exchange_helper::get_subdivision_possible_keyword($matching_data['subdivision_english_name']),
            ));
            ...
            if ($matching_data['only_subdivision']) {
                ...
                if ($redirect_component_id) {
                    $nc_core->sub_class->create($redirect_component_id, array(
                        'Subdivision_ID' => $matching_data['subdivision_id'],
                    ));
                }
            }
        }
        ...
        if (!$matching_data['only_subdivision'] && $object['format'] != nc_netshop_exchange_import::FORMAT_PRICE) {
            $subdivision_id = $matching_data['subdivision_id'];
            $component_id = $matching_data['component_id'];
            ...
            if (empty($sub_class_id)) {
                ...
                $nc_core->sub_class->create($component_id, array(
                    ...
                    'EnglishName' => nc_netshop_exchange_helper::get_sub_class_possible_keyword($matching_data['component_name'], $subdivision_id),
                ));
            }
        }

        // Заполним массив маппинга
        $object_exchange_matching[$matching_data['item_key']] = array(
            'subdivision_id' => $matching_data['subdivision_id'],
            'component_id' => !empty($matching_data['component_id']) ? $matching_data['component_id'] : null,
            'fields' => array(),
        );
        ...
        if (!$matching_data['only_subdivision']) {
            ...
            foreach ($fields_data as $index => $field_data) {
                ...
                $object_exchange_matching[$matching_data['item_key']]['fields'][$field_data['key']] = $field_data['field_id'];
            }
        }

        // Заполним массив настроек
        if (!$matching_data['only_subdivision']) {
            $skip_rows = (int)nc_array_value($matching_data, 'skip_rows', 0);
            $skip_rows  = $skip_rows >= 0 ? $skip_rows : 0;
            $object_exchange_matching[$matching_data['item_key']]['skip_rows'] = $skip_rows;
        }

        // Сохраним
        $object->set('matching', nc_array_json($object_exchange_matching));
        $object->save();

        $component_id = $object_exchange_matching[$matching_data['item_key']]['component_id'];
        if (!empty($component_id) && $matching_data['component_source'] === 'new') {
            $component_object = $nc_core->get_component($component_id);

            $component_custom_settings_defaults = array();
            ...
            if ($variants_data['variants_source'] === 'new') {
                // Поля, которые будут отличать варианты
                $fields = $variants_data['fields'];

                if (!empty($fields)) {
                    foreach ($fields as $field) {
                        $component_custom_settings_defaults['variant_fields'][$field] = array(
                            'caption' => $component_object->get_field($field, 'description'),
                            'placeholder' => NETCAT_MODULE_NETSHOP_EXCHANGE_SELECT. ' ' . nc_lcfirst($component_object->get_field($field, 'description')),
                        );
                    }
                    $component_custom_settings_defaults['variant_fields'] = nc_array_json($component_custom_settings_defaults['variant_fields']);
                }
            }

            // Обновим у компонента поля, определяющие поля для фильтра
            $search_data = $this->input->fetch_post('search');
            if ($search_data['search_source'] === 'new') {
                // Поля, которые будут в фильтре (упрощённый вариант настройки фильтра — только перечисление полей)
                $component_custom_settings_defaults['filter_fields'] = nc_array_json(array_values($search_data['fields']));
            }

            // Сохранение полей вариантов и фильтра в настройках компонента
            // @todo сейчас сохраняется в initial_value, но когда у компонентов появится
            // возможность настраивать списки полей на уровне компонентов, должно будет
            // сохраняться в default_values
            if ($component_custom_settings_defaults) {
                $this->save_component_custom_settings_initial_values($component_id, $component_custom_settings_defaults);
            }
        }

        $object->matching->next();
        // Если замаппили все файлы
        $phase++;
        if ($phase <= $object->matching->total()) {
            $this->redirect_to_index_action('matching', "&object_id={$object_id}&phase={$phase}");
        } else {
            $object->matching->finish();
    ...
    public function action_report($action, $site_id, $object_id = -1, $phase = 1) {
        ...
        if (!$exchange_id) {
            $this->redirect_to_index_action('index');
        }
    ...
    /**
     * Изменяет параметры дефолтных значений пользовательских настроек у компонента
     * @param $component_id
     * @param array $component_custom_settings_defaults
     */
    protected function save_component_custom_settings_initial_values($component_id, array $component_custom_settings_defaults) {
        $nc_core = nc_core::get_object();
        $settings_string = $nc_core->component->get_by_id($component_id, 'CustomSettingsTemplate');
        $settings_array = array(); // будет получен в eval
        eval($settings_string); // установит $settings_array
        foreach ($component_custom_settings_defaults as $field => $value) {
            if (isset($settings_array[$field])) {
                $settings_array[$field]['initial_value'] = $value;
            }
        }
        $new_settings_string = '$settings_array = ' . var_export($settings_array, true) . ';';
        nc_db_table::make('Class')->where_id($component_id)->update(array(
            'CustomSettingsTemplate' => $new_settings_string,
        ));
    }
