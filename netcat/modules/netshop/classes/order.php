    ...
    static public function from_post_data(array $post_data, nc_netshop $netshop) {
        ...
        foreach ($post_data as $field => $value) {
            if (strpos($field, "f_") === 0) {
                ...
                if ($order_component->has_field($field_name)) {
        ...
        if (nc_core::get_object()->inside_admin && isset($post_data['items']) && is_array($post_data['items'])) {
...
//            'User_ID' => nc_core::get_object()->user->get_current('User_ID'),
    ...
    public function set_delivery_point_id($delivery_point_id) {
        $delivery_method = $this->get_delivery_method();
        if ($delivery_method && $delivery_point_id && $delivery_method->get_delivery_type() === nc_netshop_delivery::DELIVERY_TYPE_PICKUP) {
    ...
    public function get_items_with_distributed_cart_discount($use_currency_rounding_setting = false) {
        ...
        $min_price = $netshop->get_setting('AllowZeroPrice') ? 0 : $price_unit;

        $totals = $order_items->sum('TotalPrice');

        $result = clone $order_items;
        // после распределения скидок позиции могут стать неуникальными (один товар с разной ценой):
        $result->set_index_property(null);
        ...
        foreach ($result as $item) {
            ...
            // и не больше стоимости товара или остатка скидки:
            $discount = min($discount, $item['ItemPrice'] - $min_price, $discount_remainder);
        ...
        // Если остался нераспределённый остаток (или распределённая скидка больше скидки на корзину), корректируем цену одного из товаров
        if (abs($discount_remainder) >= $price_unit) {
            // Сначала пробуем добавить остаток скидки к товарам с количеством 1
            foreach ($result as $item) {
                if ($item['Qty'] == 1 && $item['ItemPrice'] - $discount_remainder >= $min_price) {
                    $item->add_custom_discount(NETCAT_MODULE_NETSHOP_CART_DISCOUNT, $discount_remainder);
                    return $result; // <--- RETURN ---
                }
            }

            // Если нет товара в 1 экземпляре, делим один из товаров на 2 позиции
            foreach ($result as $item) {
                if ($item['Qty'] > 1 && $item['ItemPrice'] - $discount_remainder >= $min_price) {
                    $item['Qty'] -= 1;
                    unset($item['TotalPrice']);

                    /** @var nc_netshop_item $item_duplicate */
                    $item_duplicate = clone $item;
                    $item_duplicate['Qty'] = 1;
                    $item_duplicate->add_custom_discount(NETCAT_MODULE_NETSHOP_CART_DISCOUNT, $discount_remainder);
                    $result->unshift($item_duplicate);

                    return $result; // <--- RETURN ---
                }
            }
            // Если не нашли подходящего товара, то какая-то часть скидки остались нераспределённой :(
        }
