...
class nc_netshop_market_admin_controller extends nc_netshop_admin_controller
{
  ...
  protected function action_edit()
  {
    ...
    $delivery_methods = array('0' => NETCAT_MODULE_NETSHOP_EXCHANGE_FAKE_FIELD_NO_FIELD);
    foreach (nc_netshop::get_instance()->delivery->get_all_methods() as $delivery_method) {
      if (!$delivery_method->get('handler_id')) {
        $delivery_methods[$delivery_method->get('id')] = $delivery_method->get('name');
      }
    }
    ...
    $view = $this->view('bundle_edit')
            ...
            ->with('default_delivery_courier', $delivery_methods)
    ;
