    ...
    if (is_array($structure) && count($structure) > 0) {
      foreach ($structure as $category) {
        $this->all_sections_id[$category['Class_ID']][] = $category['Subdivision_ID'];
      	...
      	if (in_array($category['Subdivision_ID'], $processed_categories, true)) {
      		continue;
      	}

        $processed_categories[] = $category['Subdivision_ID'];
  ...
  public function get_export_bundle_offers($bundle_id, $domain)
    {
        ...
        $bundle_delivery = new $this->bundle_class($bundle_id);
        if ($bundle_delivery['delivery_courier_method_id']) {
            try {
                $method = new nc_netshop_delivery_method($bundle_delivery['delivery_courier_method_id']);
                if ($method->get('extra_charge_absolute')) {
                    $delivery_options_days = $method->get('minimum_delivery_days');
                    if ($method->get('maximum_delivery_days')) {
                        if ($delivery_options_days) {
                            $delivery_options_days .= "-";
                        }
                        $delivery_options_days .= $method->get('maximum_delivery_days');
                    }
                    $shipment_time_array = array();
                    if ($method->get('shipment_time')) {
                        $shipment_time_array = explode(":", $method->get('shipment_time'));
                    }

                    $ret_offer[] = "\t\t<delivery-options>\n";
                    $ret_offer[] = "\t\t\t<option cost=\"" . $method->get('extra_charge_absolute') . "\" days=\"" . $delivery_options_days . "\"" . ((int)$shipment_time_array[0] ? " order-before=\"" . (int)$shipment_time_array[0] . "\"" : "") . "/>\n";
                    $ret_offer[] = "\t\t</delivery-options>\n";
                }
            }
            catch (Exception $e) {
                echo NETCAT_MODULE_NETSHOP_DELIVERY_METHOD_INCORRECT_METHOD_ID;
            }
        }
                ...
                foreach ($this->export_xml_fields as $field => $attrs) {
                    ...
                    if (empty($attrs['multi'])) {
                        if ($attrs['editable'] == true && !empty($this->map_values[$field])) {
                            if ($attrs['parent_field']) {                                
                                if ($row[$this->netcat_fields[$this->map_values[$field]]]) {
                                    $fields_data[$attrs['parent_field']][$field] = $row[$this->netcat_fields[$this->map_values[$field]]];
                                }
                                continue;
                            }
                            if ($field === 'currencyId') {
                            ...
                            switch ($field) {
                                ...
                                case 'delivery_options':
                                    if ($fields_data['delivery_options']) {
                                        $ret_offer[] = "\t\t\t\t<delivery-options>\n";
                                        $ret_offer[] = "\t\t\t\t\t<option cost=\"" . $fields_data['delivery_options']['delivery_options_cost'] . "\" days=\"" . $fields_data['delivery_options']['delivery_options_days'] . "\"" . ($fields_data['delivery_options']['delivery_options_order_before'] ? " order-before=\"" . $fields_data['delivery_options']['delivery_options_order_before'] . "\"" : "") . "/>\n";
                                        $ret_offer[] = "\t\t\t\t</delivery-options>\n";
                                    }
                                    break;
                            }
