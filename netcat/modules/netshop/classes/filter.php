...
class nc_netshop_filter {
    ...
    const MACROFUNC_NAME = 'NETCAT_NETSHOP_FILTER_FORM';
    ...
    public function __construct(nc_netshop $netshop) {
        ...
        // Макрофункция NETCAT_NETSHOP_FILTER_FORM не будет ничего выводить,
        // если в инфоблоке с товаром не был выполнен вызов $netshop->filter->init_form_macro()
        $nc_core->register_macrofunc(self::MACROFUNC_NAME, function() {});
    }
        ...
        foreach ($fields as $name => $field) {
            ...
            if ($f_current) {
                ...
                if ($lookup_type == self::VARIANT_VALUES_IN_SUBQUERY) { // для query_where в списке компонентов
                    ...
                    $query .= " AND " . str_replace("a.`{$name}`", $this->get_variant_value_sql($name, $field['raw_type']), $field_query) . " ";
                }
     ...
     * @param int|string $field_type
     * @return string
     */
    protected function get_variant_value_sql($field_name, $field_type) {
        if ($field_type == NC_FIELDTYPE_SELECT) {
            // NULL, 0 → наследовать
            $child_field_has_value = "`child`.`$field_name`";
        } else if ($field_type == NC_FIELDTYPE_MULTISELECT) {
            // NULL, ',', '', '0' → наследовать
            $child_field_has_value = "TRIM(',' FROM `child`.`$field_name`)";
        } else {
            // NULL, '' → наследовать
            $child_field_has_value = "LENGTH(`child`.`$field_name`)";
        }

        return "IF($child_field_has_value, `child`.`$field_name`, `parent`.`$field_name`)";
    }
    ...
    public function make_form($fields = array()) {
        ...
        foreach ($fields as $name => $field) {
            ...
            if ($type == 'range') {
                if (isset($field_data['range'])) {
                    ...
                    $range = $this->get_field_values_range($name, $field_data['raw_type'], $apply_filter);
                }
        ...
        $result = '<div class="nc_netshop_filter_fieldset">' . $result . '</div>' .
                  ...
                  '<button class="nc_netshop_filter_submit tpl-button-primary" type="submit">' . NETCAT_MODULE_NETSHOP_FILTER_SHOW . '</button> ' .
                  '<button class="nc_netshop_filter_reset tpl-button-secondary" type="submit" name="nc_filter_reset" value="1">' . NETCAT_MODULE_NETSHOP_FILTER_RESET . '</button>' .
                  '</div>';
        ...
        if ($form_method === 'GET') {
            ...
            foreach ($this->flatten_array($get_data) as $key => $value) {
                $get_hidden_fields[] =
    ...
    /**
     * Делает массив одномерным
     * @param array $array
     * @param string $prefix
     * @return array
     */
    protected function flatten_array(array $array, $prefix = '') {
        $flat = array();
    ...
    public function make_field($name) {
        ...
        $skip_current_field = !$show_without_choice && $field['options'] && count($field['options']) < 3;
        ...
        switch ($field['field']) {
            ...
            case 'range':
                ...
                $apply_filter = $this->should_apply_filter_to_range_values($name);
                $result_range = $this->get_field_values_range($name, $field['raw_type'], $apply_filter);
        ...
        if ($result['field'] == 'checkbox' || $result['field'] == 'select' || $result['field'] == 'multiple' || $result['field'] == 'link') {
            ...
            if ( ! $options) {
                ...
                if ($field['format'] && $field['type'] == NC_FIELDTYPE_SELECT || $field['type'] == NC_FIELDTYPE_MULTISELECT) {
                    ...
                    $options = $this->get_field_possible_values($name, $result['raw_type']);
                }
            }
...
//                : array(0,0);
     ...
     * @return mixed
     */
    protected function get_field_values_range($field_name, $field_type, $filtered_only = false) {
        static $range = array();

        $key = (int)$filtered_only;

        if (!isset($range[$field_name][$key])) {
            ...
            $variant_field_value = $this->get_variant_value_sql($field_name, $field_type);
            ...
            $sql = "SELECT MIN(t.`min`), MAX(t.`max`) FROM (
                        (SELECT MIN(`a`.`$field_name`) AS `min`, MAX(`a`.`$field_name`) AS `max`
                           FROM `$table` AS `a`
                          WHERE `a`.`Checked` = 1
            ...
            $range[$field_name][$key] = $result;
        }

        return $range[$field_name][$key];
    }
     ...
     * @return mixed
     */
    protected function get_field_possible_values($field_name, $field_type) {
        static $list = array();
        ...
        if (!isset($list[$field_name])) {
            ...
            if ($this->filter_data && $this->options('filter_values') && !$this->field_value($field_name)) {
                $fields = $this->init_fields();
                unset($fields[$field_name]);
            ...
            $sql = "(SELECT DISTINCT `a`.`$field_name` AS `value`
                       FROM `$table` AS `a`
                      WHERE `a`.`Checked` = 1
                        AND `a`.`Parent_Message_ID` = 0
                            $main_item_where)
    ...
    /**
     * Инициализирует в компоненте товара форму фильтра, которая будет отображена на страницах
     * сайта в отдельном инфоблоке.
     *
     * (Метод существует, чтобы не перегружать шаблоны компонентов повторяющимся кодом
     * для добавления макрофункции; при необходимости реализовать «нестандартную» форму
     * аналогичный код можно использовать непосредственно в шаблоне компонента товара.)
     */
    public function init_form_macro() {
        $nc_core = nc_core::get_object();

        // $this->get_infoblock_id() не подходит — может быть false:
        $goods_infoblock_id = $nc_core->sub_class->get_current('Sub_Class_ID');
        // нужно сгенерировать сейчас, так как потом фильтр увидит другой $cc и не сможет создать форму:
        $filter_form = $this->make_form();
        // в PHP 5.3 не получится использовать $this внутри замыкания:
        $filter = $this;

        $nc_core->register_macrofunc(
            self::MACROFUNC_NAME,
            // В %NETCAT_NETSHOP_FILTER_FORM% должны быть переданы ID инфоблока фильтра и REQUEST_TIME
            function($filter_infoblock_id, $request_time) use ($goods_infoblock_id, $filter_form, $filter) {
                // проверка REQUEST_TIME — перестраховка от вставки формы фильтра туда, где его не должно быть
                if ($request_time != ($_SERVER['REQUEST_TIME'] ?: 1)) {
                    return '';
                }

                $content = '';
                // Оверлей, открывающий настройки фильтра в режиме редактирования
                if (nc_core::get_object()->admin_mode) {
                    $filter_settings_dialog_url = $filter->get_settings_dialog_url($goods_infoblock_id, $filter_infoblock_id);
                    $content .= nc_modal_dialog_trigger($filter_settings_dialog_url, 'xlarge');
                    if (!$filter_form) {
                        // если поля не выбраны (формы нет), покажем подсказку, что тут мог бы быть фильтр
                        $content .=
                            "<div class='nc-netshop-filter-hint-for-admin'>" .
                            NETCAT_MODULE_NETSHOP_FILTER_SETTINGS .
                            "</div>";
                    }
                }
    ...
    /**
     * Возвращает URL для открытия диалога с настройками фильтра товаров для режима редактирования
     * @param $goods_infoblock_id
     * @param $filter_infoblock_id
     * @return string
     */
    public function get_settings_dialog_url($goods_infoblock_id, $filter_infoblock_id) {
        $nc_core = nc_core::get_object();
        // далее идут некоторые знания об устройстве шаблона компонента товаров
        // (пользовательские настройки компонента, кастомный диалог их редактирования),
        // но они всё же добавлены сюда, чтобы не перегружать шаблоны компонентов
        // и была возможность легко изменить это при необходимости
        return $nc_core->SUB_FOLDER . $nc_core->HTTP_ROOT_PATH . 'action.php?' .
            http_build_query(array(
                'ctrl' => 'admin.infoblock',
                'action' => 'show_settings_dialog',
                'infoblock_id' => $goods_infoblock_id,
                'filter_settings' => 1,
    ...
    /**
     * Возвращает макрос, необходимый для вставки формы, инициализированной через
     * $netshop->filter->init_form_macro()
     *
     * @param int $filter_infoblock_id
     * @return string
     */
    static public function insert_form_macro($filter_infoblock_id) {
        $macro = self::MACROFUNC_NAME;

        // [Заодно] регистрируем функцию, чтобы не выводился код вызова макроса, когда на
        // странице нет блока с товарами, который бы вызывал $netshop->filter->init_form_macro():
        $nc_core = nc_core::get_object();
        if (!$nc_core->has_macrofunc($macro)) {
            $nc_core->register_macrofunc($macro, function() {});
        }

        $t = ($_SERVER['REQUEST_TIME'] ?: 1);
        return "%$macro($filter_infoblock_id, $t)%";
    }
