...
class nc_infoblock_controller extends nc_ui_controller {
    ...
    protected function action_save() {
        ...
        if ($custom_settings || $nc_core->input->fetch_files('custom_settings')) {
            $field_settings = $nc_core->sub_class->get_by_id($infoblock_id, 'CustomSettingsTemplate');
    ...
    protected function action_show_new_infoblock_dialog() {
        ...
        if ($subdivision_id && !$this->is_outside_content_area($position, $infoblock_id)) {
            ...
            $this->check_site_permissions(NC_PERM_ACTION_SUBCLASSADD);
        }
    ...
    protected function action_set_component_template() {
        ...
        if ($nc_core->sub_class->get_by_id($infoblock_id, 'AreaKeyword')) {
    ...
    protected function action_paste() {
        ...
        if ($paste_mode === 'cut' && $destination_subdivision_id != $pasted_subdivision_id && $nc_core->sub_class->disallow_move_and_delete($pasted_infoblock_id) === true) {
    ...
    protected function is_outside_content_area($position, $infoblock_id) {
        return $infoblock_id &&
               nc_core::get_object()->sub_class->get_by_id($infoblock_id, 'IsMainContainer') &&
