...
function SearchUserForm($totalUsers) {
  ...
  <form method='get' action='index.php' id='userSearchForm' style='background-color: #EEE; padding: 16px; margin-right: 16px;'>
    ...
    $html .= "<option value='".GROUP_MODERATOR."' ".(in_array(GROUP_MODERATOR, $rightsIds) ? 'selected':'').">".CONTROL_USER_RIGHTS_USER_GROUP."</option>";
    $html .= "<option value='".CLASSIFICATOR_ADMIN."' ".(in_array(CLASSIFICATOR_ADMIN, $rightsIds) ? 'selected':'').">".CONTROL_USER_RIGHTS_CLASSIFICATORADMIN."</option>";
...
function SearchUserResult() {
    ...
    /** @var Permission $perm */
    if (!empty($Users)) {
        $can_edit_any_user = $perm->isAccess(NC_PERM_ITEM_USER, NC_PERM_ACTION_EDIT, -1);  // Есть ли в принципе доступ к редактированию
        $can_delete_any_user = $perm->isAccess(NC_PERM_ITEM_USER, NC_PERM_ACTION_DEL, -1);  //                             и удалению
        $can_admin_any_user = $perm->isAccess(NC_PERM_ITEM_USER, NC_PERM_ACTION_RIGHT, -1);  //                            и к правам
        ?>
                ...
                <tr>
                    <td>
                        <table class='nc-table nc--striped nc--small' width='100%'>
                            ...
                            <?
                            // собственно вывод пользователя
                            foreach ($Users as $User) {
                                $can_edit_this_user = $perm->isAccess(NC_PERM_ITEM_USER, NC_PERM_ACTION_EDIT, $User['id']); // может ли редактировать данного пользователя
                                ...
                                if ($can_edit_any_user) {
                                    ...
                                    if ($can_edit_this_user) {
                                        if (!$is_current_user) {
                                            print "<a href=\"index.php?" . $nc_core->token->get_url() . "&phase=12&UserID=" . $User['id'] . "\">" . ($User['checked'] ? NETCAT_MODERATION_TURNTOOFF : NETCAT_MODERATION_TURNTOON) . "</a>";
                                            if (CMS_SYSTEM_NAME === 'Netcat') {
                                                print ' | ';
                                            }
                                        }
                                        if (CMS_SYSTEM_NAME === 'Netcat') {
                                            print "<a href=\"index.php?phase=6&UserID=" . $User['id'] . "\">" . CONTROL_USER_CHANGEPASS . "</a>\n";
                                        }
                                    }
                                    print "</td>\n";
                                }
                        ...
                        <table class='nc-table nc--striped nc--small' width='100%'>
                            ...
                            foreach ($groups as $group) {
                                ...
                                if ($perm->isAccess(NC_PERM_ITEM_GROUP, NC_PERM_ACTION_RIGHT, $id)) {
                                    print "<a href='$link.rights($id)' target='_top'><i class='nc-icon nc--settings nc--hovered' title='" . CONTROL_USER_CHANGERIGHTS . "'></i></a>";
                                }
                                print "</td>";
...
function GroupForm($PermissionGroupID, $action, $phase, $type) {
    ...
    <?php if ($perm->isAccess(NC_PERM_ITEM_GROUP, NC_PERM_ACTION_EDIT, $PermissionGroupID, 0)): ?>
        ...
        <form method="post" action="<?= htmlspecialchars($action) ?>">
            <?= CONTROL_USER_GROUPNAME ?>
            :<br><?= nc_admin_input_simple('PermissionGroupName', $Group['PermissionGroup_Name'], 50, "maxlength='64'") ?>
                        ...
                        <table class='admin_table permission_table' style='width:100%'>
                            ...
                            <tr>
                                                                    ...
                                                                    (($i == NC_PERM_EDIT_ID) ? CONTROL_CONTENT_CATALOUGE_FUNCS_CATALOGUEFORM_CHANGE :
                                                                            (($i == NC_PERM_CHECKED_ID) ? CONTROL_CLASS_ACTIONS_CHECKED :
                                                                                    (($i == NC_PERM_DELETE_ID) ? CONTROL_CLASS_ACTIONS_DELETE :
                                                                                            (($i == NC_PERM_SUBSCRIBE_ID) ? CONTROL_CONTENT_CATALOUGE_FUNCS_CATALOGUEFORM_SUBSCRIBE :
                                ...
                                foreach ($module_permissions as $row) {
                                    ...
                                    print "<td>" . nc_admin_checkbox_simple("deleted_module_permissions[]", $row['id'], '', false, '', "class='wh'$disabled") . "</td>\n";
                                    print "</tr>";
                                }
                    ...
                    function AddPermissionForm($UserID, $phase = 10, $action = 'index.php', $PermissionGroupID = 0) {
                        ...
                        <script language='javascript'>
                            ...
                            var some_const = {
                                ...
                                contentadmin : '<?= CONTROL_USER_RIGHTS_CONTENTADMIN ?>',
                                ccadmin: '<?= CONTROL_USER_RIGHTS_SUBCLASSADMINS ?>',
                                site : '<?= CONTROL_USER_FUNCS_SITE ?>',
                                ...
                                module: '<?= CONTROL_USER_MODULE ?>',
                                module_permission: '<?= CONTROL_USER_RIGHTS ?>'
                            }
                                ...
                                <tr><td width='31%' valign='top'>
                                        ...
                                        <fieldset>
                                            ...
                                            <? if ($perm->hasAnyClassificatorPermissions()): ?>
                                                <nobr>
                                                    <?= nc_admin_radio_simple('AdminType', CLASSIFICATOR_ADMIN, CONTROL_USER_RIGHTS_CLASSIFICATORADMIN, $AdminType == CLASSIFICATOR_ADMIN, 'devel', "onclick='nc_user_obj.setType(14)'" . $disabled) ?>
                                                    <br>
                                                </nobr>
                                            <? endif; ?>
                                            ...
                                            <? if ($perm->hasAnyModulePermission()): ?>
                                                <nobr>
                                                    <?= nc_admin_radio_simple('AdminType', 'module', CONTROL_USER_RIGHTS_MODULE, false, 'module', "onclick='nc_user_obj.setType(\"module\")'") ?>
                                                    <br>
                                                </nobr>
                                            <? endif; ?>
                                            ...
                                            <?php if ($perm->isGroupModerator()): ?>
                                                <nobr>
                                                    <?= nc_admin_radio_simple('AdminType', GROUP_MODERATOR, CONTROL_USER_RIGHTS_USER_GROUP, $AdminType == GROUP_MODERATOR, 'user_group', "onclick='nc_user_obj.setType(13)'" . $disabled) ?>
                                                    <br>
                                                </nobr>
                                            <? endif; ?>
                                        ...
                                        <div name='div_main_right' id='div_main_right' style='display: none'>
                                            <fieldset><legend><?= CONTROL_USER_RIGHTS_RIGHT ?></legend>
                                                <div name='userperm' id='userperm' style='display: none'><br>
                                                    ...
                                                    <?= nc_admin_checkbox_simple('user_moderate', 1, CONTROL_USER_RIGHTS_CONTROL_MODERATE) ?><br>
                    ...
                    function AddPermissionCompleted() {
                        ...
                        switch ($AdminType) {
                            ...
                            case CLASSIFICATOR_ADMIN:
                                $insert['PermissionSet'] = intval(MASK_READ + $Edit * MASK_EDIT + $Add * MASK_ADD + $Moderate * MASK_MODERATE); // 1 == view -  default
                    ...
                    function UpdatePermission() {
                        ...
                        $fetch_permission_data = function($permission_id) {
                            ...
                            if (!isset($cache[$permission_id])) {
                                $cache[$permission_id] = nc_db()->get_row("SELECT * FROM `Permission` WHERE `Permission_ID` = $permission_id", ARRAY_A) ?: array();
                            }
                        ...
                        foreach ($_POST AS $key => $val) {
                            ...
                            if (strpos($key, 'Delete') === 0) {
                                ...
                                if ($permission_data['User_ID'] == $perm->getUserID() && ($permission_data['AdminType'] == DIRECTOR || $permission_data['AdminType'] == SUPERVISOR)) {
...
function nc_get_module_permission_info($user_id, $group_id) {
    ...
    $data = (array)$nc_core->db->get_results(
        "SELECT * FROM `Module_Permission` WHERE `User_ID` = " . (int)$user_id . " AND `PermissionGroup_ID` = " . (int)$group_id,
        ARRAY_A
    );
    ...
    foreach ($data as $row) {
        ...
        if ($permission_type === NC_PERM_MODULE_ALL) {
            $permission_description = CONTROL_USER_RIGHTS_MODULE_ALL;
        } else {
        ...
        if ($row['PermissionBegin']) {
            $valid .= NETCAT_COND_DATE_FROM . ' ' . date(NETCAT_CONDITION_DATETIME_FORMAT, strtotime($row['PermissionBegin']));
        }

        if ($row['PermissionEnd']) {
            $valid .= ($valid ? ' ' : '') . NETCAT_COND_DATE_TO . ' ' . date(NETCAT_CONDITION_DATETIME_FORMAT, strtotime($row['PermissionEnd']));
        }
    ...
    usort($permissions, function($a, $b) { return $a['description'] < $b['description'] ? -1 : 1; });
...
function nc_get_all_module_permissions() {
    ...
    $modules = (array)nc_db()->get_results("SELECT `Keyword`, `Module_Name`, `PerSitePermissions` FROM `Module`", ARRAY_A, 'Keyword');
    ...
    // Сортировка модулей по имени
    uasort($permissions, function($a, $b) { return $a['name'] < $b['name'] ? -1 : 1; });
    ...
    foreach ($constants['user'] as $key => $value) {
        if (preg_match("/^NC_PERM_MODULE_($modules_string)_(.+)$/", $key, $matches)) {
            ...
            $permission = strtolower($matches[2]);
...
function nc_save_module_permissions_form(array $properties, array $permissions) {
    ...
    $conditions =
        ...
        " AND `Catalogue_ID` IN (0, " . (int)$permissions['Catalogue_ID'] . ")" .
        " AND `PermissionBegin`" . ($properties['PermissionBegin'] ? " = '" . $db->escape($properties['PermissionBegin']) . "'" : ' IS NULL') .
        " AND `PermissionEnd`" . ($properties['PermissionEnd'] ? " = '" . $db->escape($properties['PermissionEnd']) . "'" : ' IS NULL');
