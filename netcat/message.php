...
do {
    ...
    if ($delete) {
        ...
        /** @todo ↓ TEMPORARY! */
        if (!$user_table_mode && !$db->get_row("SHOW COLUMNS FROM `Message$classID` LIKE 'ncDemoContent'")) {
            $db->query("ALTER TABLE `Message$classID` ADD COLUMN `ncDemoContent` tinyint NOT NULL DEFAULT 0 AFTER `ncSMO_Image`");
            $db->query("ALTER TABLE `Message$classID` ADD INDEX `ncDemoContent` (`ncDemoContent`, `Subdivision_ID`)");
        }
        /** @todo ↑ TEMPORARY! */
        ...
        if ($posting == 0) {
            if (!isset($posting) && count($fld)) {
                ...
                if ($user_table_mode) {
                    ...
                    $select = "SELECT " . $fieldQuery . ", `Checked`, `Keyword`, `ncTitle`, `ncKeywords`, `ncDescription`, `ncSMO_Title`, `ncSMO_Description`, `ncSMO_Image`, `Parent_Message_ID`, `ncDemoContent`
            ...
            if ($editTemplate || ($cc_env['File_Mode'] && filesize($file_class->get_field_path('EditTemplate')))) {
                ...
                if ($cc_env['File_Mode']) {
                    ...
                    echo nc_prepare_message_form(ob_get_clean(), $action, $admin_mode, $user_table_mode, $sys_table_id, $current_cc, $f_Checked, $f_Priority, $f_Keyword, $f_ncTitle, $f_ncKeywords, $f_ncDescription, 1, 0, $f_ncSMO_Title, $f_ncSMO_Description, $f_ncSMO_Image, $f_ncDemoContent);
...
// ncSMO*
                    ...
                    if ($user_table_mode) {
                        ...
                        $nc_mode = $nc_core->catalogue->get_by_id((int)$catalogue, 'ncMode');
                        $set_ncDemoContent = '';
                        if ($perm->isSupervisor() && ($nc_mode == 'template_development' || $nc_mode == 'demo')) {
                            if ($admin_mode) {
                                $set_ncDemoContent = " `ncDemoContent` = '" . (int)$f_ncDemoContent . "', ";
                            }
                        } else {
                            $set_ncDemoContent = " `ncDemoContent` = '0', ";
                        }

                        $SQL = "UPDATE `Message" . $classID . "` SET " . $updateString .
                            ($admin_mode ? $set_checked . $set_keyword : '') . $set_ncDemoContent .
